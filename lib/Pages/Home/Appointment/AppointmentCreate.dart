...
class AppointmentCreate extends StatefulWidget {
  const AppointmentCreate({super.key});
...
class _AppointmentCreateState extends State<AppointmentCreate> {
  ...
  Future<void> _fetchCustomers() async {
    try {
      String apiUrl =
          '${ApiConfig.baseUrl}$Terant/user/AccountMyAdditional/?Created_By=$User_Id';
  ...
  Future<void> _handleSave() async {
    ...
    try {
      String apiUrl = "${ApiConfig.baseUrl}$Terant/user/UserLeadMeetingCRUD/";

      final matCustomer = CustomerData.firstWhere(
        (e) => e['CompanyName'] == selectedCustomerName,
        orElse: () => {},
      );
      int cId = matCustomer['id'] ?? 0;
      ...
      print("Appointment Create ${response.body}");
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Navigator.pop(context); // Go back to list or home
      } else {
        ErrorToast.showToast(
          context: context,
          title: 'Error',
      ...
      ErrorToast.showToast(
        context: context,
        title: 'Error',
        description: 'Network issue occurred.',
      );
    } finally {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDropdownField(
                      ...
                      items: CustomerData.map(
                        (e) => e['CompanyName'].toString(),
                      ).where((e) => e.isNotEmpty && e != 'null').toList(),
                      onChanged: (v) =>
                          setState(() => selectedCustomerName = v),
                    ),
          ...
          child: Row(
            ...
            children: [
              Text(
                ...
                style: GoogleFonts.poppins(
                  fontSize: 14,
                  color: value.contains("Select") || value.contains("--")
                      ? Colors.grey[500]
                      : Colors.black87,
                ),
              ),
